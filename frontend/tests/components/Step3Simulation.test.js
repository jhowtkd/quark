...
const mockStartSimulation = vi.fn(() => Promise.resolve({ success: true, data: { process_pid: 12345 } }))
const mockStopSimulation = vi.fn(() => Promise.resolve({ success: true }))
...
describe('Step3Simulation', () => {
  ...
  it('passes evolution config to startSimulation', async () => {
    const wrapper = mountComponent()
    await nextTick()

    // Set evolution config
    wrapper.vm.evolutionConfig = {
      enableAgentEvolution: true,
      agentEvolutionPreset: 'polarizable',
    }

    // Trigger start
    wrapper.vm.showConfirmModal = false
    await wrapper.vm.doStartSimulation()
    await nextTick()
    ...
    expect(mockStartSimulation).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        agent_evolution_preset: 'polarizable',
      })
    )
  })
})
