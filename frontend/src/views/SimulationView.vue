...
const handleGoBack = () => {
  // Retrair visual para Step de Fluxo
...
const handleNextStep = (params = {}) => {
  ...
  // Montagem Virtual Routes
  const routeParams = {
    name: 'SimulationRun',
    params: { simulationId: currentSimulationId.value }
  }
  
  // Transfer em Query String Numérica.
  if (params.maxRounds) {
    routeParams.query = { maxRounds: params.maxRounds }
  }
  
  // Caminhar aos Operadores Rumo ao Estágio Final
...
/**
 * Varredura e derrubada de ambientes legados on.
 * 当用户从 Step 3 Retroceder a Base Route / URL Push Step 2 时，默认用户要退出模拟
 */
const checkAndStopRunningSimulation = async () => {
  if (!currentSimulationId.value) return

  try {
    // Ping nas portas Engine.
    const envStatusRes = await getEnvStatus({ simulation_id: currentSimulationId.value })
    
    if (envStatusRes.success && envStatusRes.data?.env_alive) {
      addLog(t('log.detectedSimEnvRunning'))
      
      // A desligar ambiente sim com calma...
      try {
        const closeRes = await closeSimulationEnv({ 
          simulation_id: currentSimulationId.value,
          timeout: 10  // 10Timeout Limit em segundos.
        })
        ...
        if (closeRes.success) {
          ...
          // 如果优雅关闭Error Catch Target Map Array Formatting Flow Maps Action Check Displays Displays Setup Values Values Flow Output Response Return Methods Regex Variable Setup Execute Component String Map Logic Mapping Outputs String Output，尝试强制停止
          await forceStopSimulation()
        }
      } catch (closeErr) {
        addLog(t('log.closeSimEnvException', { error: closeErr.message }))
        // Corte profundo pós loop ignorado.
        await forceStopSimulation()
      }
    } else {
      // Backend operante com processos pendurados.
...
onMounted(async () => {
  ...
  // Destranca e libera ambiente regressivo base 3 -> 2）
  await checkAndStopRunningSimulation()
  
  // Injeção Base de Registrados.
  loadSimulationData()
})
</script>
