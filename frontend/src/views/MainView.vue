...
const autoStartBuild = async () => {
  // Auto flow: request preview first, then build
  // Used by "use research for graph" button in Step1GraphBuild
  await requestBuildPreview()
  if (costPreview.value) {
    await startBuildGraph()
  } else {
    graphActionError.value = 'Preview failed — cannot start graph build.'
    addLog('Error: preview failed during auto-start, manual build required.')
  }
}
