...
ESTRUTURA OBRIGATORIA (Due Diligence — exatamente 5 secoes):
1. Tese Principal — A tese central em 1-2 frases. Sem dados, tabelas ou evidencias.
2. Evidencias Verificadas — Apenas fatos com fontes e datas. OBRIGATORIO: pelo menos uma tabela quantificada.
3. Fragilidades e Riscos — Apenas riscos e contra-argumentos. NAO rebata dentro desta secao.
4. Premissas Explicitas — Apenas suposicoes criticas. Cada uma com consequencia: "Se falsa, entao...".
5. Cenarios (Bear / Base / Bull) — Projecoes quantificadas. Cada cenario deve incluir:
   - Probabilidade (%)
   - Gatilho observavel (evento concreto, nao sentimento)
   - Resultado quantificado
   - As tres probabilidades devem somar 100%

REGRAS DE CENARIOS:
- Bear: cenario pessimista. Probabilidade tipica: 20-30%.
- Base: cenario mais provavel. Probabilidade tipica: 45-55%.
- Bull: cenario otimista. Probabilidade tipica: 20-30%.
- Gatilhos devem ser eventos observaveis (ex: "Se a Selic ultrapassar 14%", nao "Se o mercado ficar nervoso").
- OBRIGATORIO: inclua tabela comparativa dos tres cenarios (Cenario | Probabilidade | Gatilho | Resultado).
...
REGRAS DE NEUTRALIDADE E EQUILIBRIO (obrigatorias):
...
- Evite linguagem que sugira certeza absoluta ("certamente", "inevitavelmente", "sem duvida"). Use gradao de confianca baseada na qualidade da evidencia.
- Ao final do relatorio, inclua obrigatoriamente uma secao "## Limitacoes Conhecidas" documentando lacunas nos dados e incertezas nao resolvidas.
""",

    "report_section_prompt": """Voce esta escrevendo a secao "{section_title}". Respeite estritamente o tipo de conteudo desta secao:

- "Tese Principal": APENAS a tese central em 1-2 frases. NENHUM dado, tabela ou evidencia.
- "Evidencias Verificadas": APENAS fatos verificados com fontes e datas. OBRIGATORIO: inclua pelo menos uma tabela (Indicador | Valor | Data | Fonte | Variacao). NENHUMA opiniao ou especulacao.
- "Fragilidades e Riscos": APENAS riscos, vulnerabilidades e contra-argumentos. NAO rebata as fragilidades nesta secao. NENHUM argumento positivo ou mitigacao.
- "Premissas Explicitas": APENAS suposicoes que sustentam a tese. Cada premissa deve ter consequencia explicita: "Se falsa, entao...". NENHUM dado ou evidencia.
- "Cenarios": APENAS projecoes quantificadas. Cada cenario (Bear, Base, Bull) deve ter: probabilidade (%), gatilho observavel, resultado quantificado. Probabilidades devem somar 100%. OBRIGATORIO: tabela comparativa.

Proibir metaforas ou adjetivos emocionais. Etiquete TODO dado quantitativo com 📊, 🔮 ou ⚠️.
""",
...
""",
    ...
    "require_bias_audit": True,

    "require_quality_gates": True,

    "outline_system_prompt": """[Planejamento de Estrutura — Due Diligence]

Voce DEVE criar exatamente 5 secoes para este relatorio, nesta ordem:

1. Tese Principal
2. Evidencias Verificadas
3. Fragilidades e Riscos
4. Premissas Explicitas
5. Cenarios (Bear / Base / Bull)

Regras:
- Use EXATAMENTE os titulos acima. Nao invente titulos alternativos.
- Nao adicione secoes extras como "Resumo Executivo" ou "Introducao".
- A descricao de cada secao deve refletir o conteudo especifico do cenario simulado.
- A secao "Cenarios" deve indicar que incluira probabilidades e gatilhos observaveis.
- Retorne JSON valido com exatamente 5 itens na lista "sections".
""",
...
class ProfileConfiguration:
    ...
    # Validacao de dados financeiros
    validation_thresholds: Dict[str, float] = field(default_factory=dict)
    require_validation: bool = False

    # Planejamento de outline
    outline_system_prompt: str = ""

    # Auditoria de vies e neutralidade
    require_bias_audit: bool = False

    # Quality gates pos-geracao
    ...
    def apply_to_report_agent(self, agent: Any) -> None:
        ...
        agent.validation_thresholds = self.validation_thresholds
        agent.require_validation = self.require_validation
        agent.outline_system_prompt = self.outline_system_prompt
        agent.require_bias_audit = self.require_bias_audit
        agent.require_quality_gates = self.require_quality_gates
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "outline_system_prompt": self.outline_system_prompt,
            "require_bias_audit": self.require_bias_audit,
