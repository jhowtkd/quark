...
LEGACY_DEFAULT_CHUNK_SIZE = 500
LEGACY_DEFAULT_CHUNK_OVERLAP = 50
...
def _normalize_chunk_settings(
    ...
    """Normalize legacy persisted defaults to the current baseline."""
    normalized_size = chunk_size if chunk_size is not None else Config.DEFAULT_CHUNK_SIZE
    normalized_overlap = (
        chunk_overlap if chunk_overlap is not None else Config.DEFAULT_CHUNK_OVERLAP
    )

    if (
        normalized_size == LEGACY_DEFAULT_CHUNK_SIZE
        and normalized_overlap == LEGACY_DEFAULT_CHUNK_OVERLAP
    ):
...
class Project:
    ...
    ontology_signature: Optional[str] = None
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "chunk_signatures": self.chunk_signatures,
            "error": self.error
        }
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'Project':
        ...
        return cls(
            ...
            ontology_signature=data.get('ontology_signature'),
            chunk_signatures=data.get('chunk_signatures', []),
