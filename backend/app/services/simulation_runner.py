...
logger = get_logger('futuria.simulation_runner')
...
class SimulationRunState:
    ...
    agent_evolution_preset: Optional[str] = None
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "agent_evolution_enabled": self.agent_evolution_enabled,
            "agent_evolution_preset": self.agent_evolution_preset,
...
class SimulationRunner:
    ...
    # Observability client (class-level for convenience)
    _observability_client: Any = None
        ...
        try:
            with open(state_file, 'r', encoding='utf-8') as f:
                ...
                state = SimulationRunState(
                    ...
                    agent_evolution_enabled=data.get("agent_evolution_enabled", False),
                    agent_evolution_preset=data.get("agent_evolution_preset"),
                    agent_evolution=data.get("agent_evolution", {}),
                )
    ...
    def start_simulation(
        ...
        Args:
            ...
            enable_agent_evolution: 是否启用Agent进化追踪
            agent_evolution_preset: Agent进化策略 (stable/sensitive/polarizable)
        ...
        if observation is not None:
            span = observation.start_span(
                name="simulation_runner.start_simulation",
                metadata={
                    "simulation_id": simulation_id,
                    "platform": platform,
        ...
        try:
            ...
            state = SimulationRunState(
                ...
                agent_evolution_enabled=enable_agent_evolution,
                agent_evolution_preset=agent_evolution_preset,
            )
    ...
    @classmethod
    def stop_simulation(cls, simulation_id: str, observation: Any = None) -> SimulationRunState:
        """停止模拟"""
        span = None
        if observation is not None:
            span = observation.start_span(
                name="simulation_runner.stop_simulation",
                metadata={"simulation_id": simulation_id},
            )
            span.update(input={"simulation_id": simulation_id})

        try:
            state = cls.get_run_state(simulation_id)
            if not state:
                if span is not None:
                    span.update(status_message="Simulation not found")
                raise ValueError(f"模拟不存在: {simulation_id}")
            
            if state.runner_status not in [RunnerStatus.RUNNING, RunnerStatus.PAUSED]:
                # Não está rodando — pode estar já parada, completar, ou nunca ter começado
                # Retorna sucesso em vez de erro, já que o usuário quer parar e já não está rodando
                result = SimulationRunState(
                    simulation_id=simulation_id,
                    runner_status=state.runner_status,
                    started_at=state.started_at,
                    completed_at=datetime.now().isoformat() if state.runner_status == RunnerStatus.COMPLETED else None
                )
                if span is not None:
                    span.update(output={"status": state.runner_status.value, "already_stopped": True})
                return result
            ...
            if span is not None:
                span.update(output={"status": state.runner_status.value})

            return state
        except Exception as e:
            if span is not None and not getattr(span, "_ended", False):
                span.update(status_message=str(e))
            raise
        finally:
            if span is not None:
