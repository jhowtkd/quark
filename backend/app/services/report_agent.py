...
class ReportAgent:
    ...
    def __init__(
        ...
        Args:
            ...
            agent_evolution: Optional agent evolution data for evidence-grounded reporting
        """
        ...
        # Build system prompt with evolution evidence instructions (after profile)
    ...
    def _build_system_prompt(self) -> str:
        """Build system prompt with agent evolution evidence rules."""
        base_prompt = getattr(self, 'system_prompt', '') or ''

        evolution_rules = """
## Agent Evolution Evidence Rules

When discussing agent behavioral changes:
1. CITE event causes for any behavioral-change claim using the format: [cause: EVENT_NAME]
2. If agent evolution data is missing or empty, state that evidence is insufficient
3. Do NOT infer psychological change from persona alone — only from recorded events
4. Use metric deltas as quantitative support when available

Agent Evolution Data:
"""

        if self.agent_evolution:
            summary = self.agent_evolution.get('summary', {})
            averages = summary.get('averages', {})
            top_changed = summary.get('top_changed_agents', [])

            evolution_data_text = f"""
- Averages: {json.dumps(averages, ensure_ascii=False)}
- Top Changed Agents: {json.dumps(top_changed, ensure_ascii=False)}
- Total Events: {len(self.agent_evolution.get('events', []))}
- Total Snapshots: {len(self.agent_evolution.get('snapshots', {}))}
"""
            evolution_rules += evolution_data_text
        else:
            evolution_rules += "\n[NO EVOLUTION DATA AVAILABLE — state that evidence is insufficient]\n"

        return f"{base_prompt}\n\n{evolution_rules}" if base_prompt else evolution_rules
...
class ReportManager:
    ...
    def _get_report_folder(cls, report_id: str) -> str:
        """getreportfolder"""
        validate_safe_id(report_id, name="report_id")
