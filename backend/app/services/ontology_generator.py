...
**NUNCA crie tipos de entidade para: conceitos abstratos (teoria, metrica, indicador), eventos (crise, guerra, eleicao), localizacoes (pais, cidade), tecnologias (algoritmo, software), produtos (medicamento, vacina).**

**Se o texto mencionar tais itens, eles devem ser modelados como ATRIBUTOS ou RELACOES de um ator, nunca como entidade independente.**

### Exemplos negativos (anti-patterns) — NUNCA faca isso:

- `"Inflacao"` → NAO crie tipo `Concept`; associe a `Economist` ou `GovernmentAgency`.
- `"Pandemia"` → NAO crie tipo `Event`; associe a `Doctor` ou `Hospital`.
- `"Sao Paulo"` → NAO crie tipo `Location`; use como atributo `location` de `Person`.
...
3. 前8个是根据文本内容设计的具体类型，且必须是 atores sociais（pessoas, organizacoes, grupos representativos）
4. 所有实体类型必须是现实中可以发声的主体，不能是抽象概念
5. 属性名不能使用 name、uuid、group_id 等保留字，用 full_name、org_name 等替代
6. 禁止为 conceitos abstratos, eventos, localizacoes, tecnologias 或 produtos 创建独立实体类型；它们应作为 atributos ou relacoes de atores
"""
    ...
    def generate_python_code(self, ontology: Dict[str, Any]) -> str:
        ...
        code_lines = [
            ...
            'Auto-generated by FUTUR.IA for social opinion simulation',
            '"""',
            '',
