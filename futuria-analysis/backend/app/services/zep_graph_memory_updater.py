...
logger = get_logger('futuria.zep_graph_memory_updater')
...
class ZepGraphMemoryUpdater:
    ...
    # Circuit breaker配置 - 连续失败5次后禁用
    CIRCUIT_BREAKER_THRESHOLD = 5
    CIRCUIT_BREAKER_RESET_DELAY = 60  # 秒后重置
    ...
    def __init__(self, graph_id: str, api_key: Optional[str] = None):
        ...
        self._circuit_broken_time = 0  # 上次失败时间
    ...
    def _build_payload_batches(self, activities: List[AgentActivity]) -> List[str]:
        """按 Zep 单次文本大小限制拆分 payload。"""
        payloads: List[str] = []
        ...
        for activity in activities:
            ...
            if len(text) > self.MAX_TEXT_PAYLOAD_CHARS:
                text = text[: self.MAX_TEXT_PAYLOAD_CHARS - 1].rstrip() + "…"

            additional_length = len(text) if not current_parts else len(text) + 1
            if current_parts and current_length + additional_length > self.MAX_TEXT_PAYLOAD_CHARS:
                payloads.append("\n".join(current_parts))
                current_parts = [text]
    ...
    def _send_batch_activities(self, activities: List[AgentActivity], platform: str):
        ...
        if self._circuit_broken:
            if time.time() - self._circuit_broken_time >= self.CIRCUIT_BREAKER_RESET_DELAY:
                logger.info(f"Circuit breaker重置，重新尝试发送到Zep图谱 {self.graph_id}")
                self._circuit_broken = False
                self._consecutive_failures = 0
            else:
                self._failed_count += 1
                logger.debug(f"Circuit breaker激活，跳过发送到Zep")
                return
        
        payload_batches = self._build_payload_batches(activities)
        display_name = self._get_platform_display_name(platform)

        for combined_text in payload_batches:
            # 带重试的发送
            for attempt in range(self.MAX_RETRIES):
                try:
                    ...
                    logger.info(f"成功发送{display_name}活动批次到图谱 {self.graph_id}")
                    ...
                    if attempt < self.MAX_RETRIES - 1:
                        ...
                        # 检查是否需要启用circuit breaker
                        if self._consecutive_failures >= self.CIRCUIT_BREAKER_THRESHOLD:
                            self._circuit_broken = True
                            self._circuit_broken_time = time.time()
                            logger.warning(f"连续{self._consecutive_failures}次失败，circuit breaker已激活，{self.CIRCUIT_BREAKER_RESET_DELAY}s后重置")
            else:
                continue
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "circuit_broken": self._circuit_broken,      # Circuit breaker状态
            "consecutive_failures": self._consecutive_failures,  # 连续失败次数
        }
