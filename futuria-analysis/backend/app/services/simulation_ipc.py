...
logger = get_logger('futuria.simulation_ipc')
...
class SimulationIPCClient:
    ...
    def __init__(self, simulation_dir: str, observability_client: Any = None):
        """
        初始化IPC客户端

        Args:
            simulation_dir: 模拟数据目录
            observability_client: Optional Langfuse observability client
        """
    ...
    def send_command(
        ...
        if observation is not None:
            span = observation.start_span(
                name="simulation_ipc.send_command",
                metadata={
                    "command_type": command_type.value,
                    "simulation_dir": self.simulation_dir,
                },
            )
            span.update(input={"command_type": command_type.value, "args": args})
        ...
        while time.time() - start_time < timeout:
            if os.path.exists(response_file):
                try:
                    ...
                    if span is not None:
                        span.update(output={"status": response.status.value, "command_id": command_id})
                        span.end()
        ...
        if span is not None:
            span.update(status_message="IPC timeout")
            span.end()
